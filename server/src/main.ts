    ...
    pc1.onicecandidate = (candidate: any) => {
      if (!candidate.candidate) {
        ...
        if (isUDP(candidate.candidate.candidate)) {
          console.log(chalk.cyanBright(`${channelId} pc1.onicecandidate (before)`), JSON.stringify(iceCandidate, null, 2))
          if (DO_MAPPING) mapPorts(iceCandidate)
          if (DO_MAPPING) console.log(chalk.cyanBright(`${channelId} pc1.onicecandidate (after)`), JSON.stringify(iceCandidate, null, 2))
